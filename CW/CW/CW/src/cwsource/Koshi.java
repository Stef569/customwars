           ...
           for(int i = 0; i < allArmies.length; i++)
           {
              ...
              if(testArmy.getSide() != army.getSide())
              {
                 ...
                 if(currCO != null)
                 {
                    ...
                    currCO.negativeLuck += 15;
                 }

                 if(altCO != null)
                 {
                    altCO.positiveLuck += 15;
                    altCO.negativeLuck += 15;
                 }
              }
           }
        }
              ...
              if(testArmy.getSide() != army.getSide())
              {
                 ...
                 if(allUnits != null)
                 {
                    for(int j = 0; j < allUnits.length; j++)
                    {
                       ...
                       testUnit.enemyCOstore[statIndex][0] = testUnit.vision - 1;
                       testUnit.vision = 1;
              ...
              if(testArmy.getSide() != army.getSide())
              {
                 ...
                 if(allUnits != null)
                 {
                    for(int j = 0; j < allUnits.length; j++)
                    {
                       ...
                       if(army.getCO() == this)
                       {
                          testUnit.vision = 1 + testUnit.enemyCOstore[statIndex][0];
                          testUnit.enemyCOstore[statIndex][0] = 0;
                          testUnit.changed = false;
                       }
                       else
                       {
                          testUnit.vision = 1 + testUnit.altEnemyCOstore[statIndex][0];
                          testUnit.altEnemyCOstore[statIndex][0] = 0;
                          testUnit.changed = false;
                       }
                    }
                 }
