...
//used to deactivate Blandie's Super CO Power the next day
    ...
    public void beforeAttack(Unit owned, Unit enemy, int damage, boolean attack) {
        endure = false;
        if(!attack && enemy.displayDamageCalc(owned) > owned.hP && owned.getDisplayHP() != 10)
            endure = true;
        //D2D
        
        if(COP && enemy.displayDamageCalc(owned) > owned.COstore[0] && !attack)
        {
            
            owned.COstore[1] = enemy.displayDamageCalc(owned) - owned.COstore[0];
            enemy.getArmy().getCO().damagePenalty += owned.COstore[1];
        }
        //COP ability ^^
        
        if(damage>owned.getHP() && !attack && SCOP) //If Napoleon is blocking and will take lethal damage
        {
            if(damage<owned.getHP()+owned.COstore[0]) { // If the damage is less than the store
    ...
    public void afterAction(Unit u, int index, Unit repaired, boolean main) {
        ...
        if(main && index == 2 && SCOP) {
            ...
            if(P.getCapturePoints() > u.COstore[0]/10 && P.getOwner() != army) {
                P.setCapturePoints(P.getCapturePoints() - u.COstore[0]/10);
            } else {
