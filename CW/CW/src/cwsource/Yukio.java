        ...
        for(int i = 0; i< armies.length; i++) {
            ...
            if(armies[i].getSide()!=army.getSide()) {
                for(int s = 0; s<u.length; s++) {
                    if(u[s].enemyCOstore[statIndex][0]%10 == 1) {
                        u[s].damage(40, false);
                        u[s].paralyzed = true;
                    }
                }
...
//used to deactivate Blandie's Super CO Power the next day
    ...
    public void afterEnemyAction(Unit u, int index, Unit repaired, boolean main) {
        ...
        if(index == 15) {
            if(COP || (sustain && !main))
                u.damage(20,false);
            u.enemyCOstore[statIndex][0] = 11; //Used for SCOP calculation.
            u.repairMod += 0.5;
        }
    }
    ...
    public void dayEnd(boolean main) {
        ...
        if(main) {
            ...
            for(int i = 0; i< armies.length; i++) {
                ...
                if(armies[i].getSide()!=army.getSide()) {
                    for(int s = 0; s<u.length; s++) {
                        if(u[s].enemyCOstore[statIndex][0] < 10)
                        {
                        u[s].repairMod += .5;
                        u[s].enemyCOstore[statIndex][0] +=10;
                        }
                    }
                }
            }
