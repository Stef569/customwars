...
public Ozzy() {
    ...
    public int getDef(Unit attacker, Unit defender){
            if(SCOP)
            {
                return (130+defender.COstore[0]*40);
            }
            else if (COP)
            {
                if (store)
                    return (152 + defender.COstore[0]*10); //This comes out to 60% reduction, on top of the 120 defense.
                                                        //Trust me >_>
                else
                    return (120+defender.COstore[0]*10); //10 point defense boost if the unit has attacked the turn before
            }
            else
            {
                return (100+defender.COstore[0]*10);
            }
    }
    ...
    public void beforeAttack(Unit owned, Unit enemy, int damage, boolean attack)
    {
        ...
        {
            if(SCOP)
                if(owned.COstore[0] == 1) //If the unit had attacked
                    counterAttack = 200;
        }
    }
