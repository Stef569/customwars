...
public class Army implements Serializable{
    ...
    public Army(int id, CO co, CO altCO, int side, int color, Battle b){
        ...
        this.id = id;
        this.co = co;
        this.co.setStatIndex(id);
        this.altCO = altCO;
        this.co.setArmy(this);
        if(altCO != null){this.altCO.setArmy(this); this.altCO.setStatIndex(id);}
        this.side = side;
        this.color = color;
        funds = 0;
        this.battle = b;
    ...
    public boolean canTag(){
        if(altCO != null && co.canSCOP() && altCO.canSCOP() && !co.isSCOP() && !co.isCOP())
            return true;
        return false;
    }
    ...
    public void swap(){
        ...
        for(int i = 0; i<a.length; i++)
        {
            if(a[i].getSide() != this.getSide())
            {
                ...
                for(int t= 0; t<a[i].getNumberOfUnits(); t++)
                {
                    for(int s = 0; s < 10; s++)
                    {
                        storage2[t] = u[t].getAltEnemyCOstore()[this.co.getStatIndex()];
                        u[t].getAltEnemyCOstore()[this.co.getStatIndex()] = u[t].getEnemyCOstore()[this.co.getStatIndex()];
                        u[t].getEnemyCOstore()[this.co.getStatIndex()] = storage2[t];
                    }
                }
            }
        }
        ...
        setChangeAll();
        changeAllProp();
        logger.info(co.getName() + ": " + co.getSwap()[battle.getRNG().nextInt(2)]);
    ...
    public void tagBreak(){
        int i = isSpecTag();
        if(i > -1){
        	logger.info(""+co.getTagNames()[i] + "!");
            atkPercent = (co.getTagPercent()[i] - 100);
    ...
    public int isSpecTag(){
        if(altCO != null && co.getTagCOs() != null)
            for(int i = 0; i < co.getTagCOs().length; i++)
                if(co.getTagCOs()[i] == altCO.getName())
                    return i;
        return -1;
    }
    ...
    public void addTerrCosts_terrain(double[] costInc, int terrID, int... terrIDarray)
    {
    	for(int col = 0; col < getBattle().getMap().getMaxRow(); col++)
    	{
        	for(int row = 0; row < getBattle().getMap().getMaxCol(); row++)
        	{
        		if(getBattle().getMap().onMap(col, row))
        		{
	        		int terrIndx = getBattle().getMap().find(new Location(col, row)).getTerrain().getIndex();
