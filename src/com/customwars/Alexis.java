...
//used to get the attack bonus for damage calculation
    public int getAtk(Unit attacker, Unit defender){
        ...
        Property[] prop = army.getProperties();
            ...
            {
                    storage = prop[i].tile.getLocation();
                    t = 0;
            ...
            if(u!=null)
                for(int i = 0; i < u.length; i++)
                {
                    if(u[i].getClass() != null)
                    {
                        ...
                        {
                            if(u[i].COstore[0] == 0) //Has the unit been healed?
                            {
                            storage = prop[itwo].tile.getLocation();
                                if(u[i].checkAdjacent(storage))
                                {
                                    if(!u[i].isInTransport())
                                    {
                                        u[i].heal(10);
                                        u[i].COstore[0] = 1; //If it has been healed, pre
