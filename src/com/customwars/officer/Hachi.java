...
public class Hachi extends CO{
    ...
    public void afterAction(Unit u, int index, Unit repaired, boolean main) {
        if(main && index == 15 && SCOP && army.getBattle().getMap().find(u).getTerrain().isUrban() && army.getBattle().getMap().find(u).getTerrain().getName().equals("City"))   
        {
            //if this CO is in front, he builds a unit, the SCOP is active and the location is a city
            if(army.getFunds() < 95*u.getPrice()/costMultiplier-u.getPrice())
            {
                army.addFunds(u.getPrice());
                u.damage(100,true);
            }
            else
            {
                army.removeFunds(95*u.getPrice()/costMultiplier-u.getPrice());
            }
        }
    }
}