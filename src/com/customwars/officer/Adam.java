...
//used to get the attack bonus for damage calculation
    public int getAtk(Unit attacker, Unit defender){
        ...
        if(COP) {
            ...
            for(int i = 0; i<attacker.getCOstore()[0]; ++i) {
                bonus+= 10;
            }
        }
        if(SCOP) {
        ...
        if(defender != null) {
            ...
            if(defender instanceof Submarine && defender.isDived())baseDamage = ((Submarine)defender).getDivedDamage(attacker.getUType());
            else
                if(defender instanceof Stealth && defender.isDived())baseDamage = ((Stealth)defender).getHiddenDamage(attacker.getUType());
...
//used to get the defense bonus for damage calculation
    public int getDef(Unit attacker, Unit defender){
        ...
        for(int i = 0; i<defender.getCOstore()[0]; ++i) {
            if(i == 1)
                bonus += 15;
            else
                bonus += 10;
    ...
    public void deactivateCOP(){
        ...
        for(int i = 0; i< u.length; i++) {
            if(u[i].getCOstore()[0] > 0) {
                for(int s = 0; s<u[i].getCOstore()[0]; s++) {
                    u[i].setMove(u[i].getMove() - 1);
                }
            }
        }
...
//used to deactivate Blandie's Super CO Power the next day
    ...
    public void afterAction(Unit u, int index, Unit repaired, boolean main) {
        if(index == 1 && SCOP && selecting) {
            ...
            u.getCOstore()[1] += (usedMP);
            
            u.setMove(remainingMP);
            
            
            u.setActive(true);
    		logger.info("" + u.getCOstore()[1]);
        }
    }
    ...
    public void selectAction(Tile t) {
        if(COP && t.hasUnit() && t.getUnit().getArmy().getSide() == army.getSide()) {
            t.getUnit().getCOstore()[0] +=1;
            t.getUnit().setMove(t.getUnit().getMove() + 1);
        }
        if(SCOP) {
            if(t.hasUnit() && t.getUnit().getArmy().getSide() == army.getSide()) {
                t.getUnit().getCOstore()[2] += 15;
            }         }
        selecting = false;
    }
