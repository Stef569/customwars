...
public class Peter extends CO{
    ...
    public Peter() {
        ...
        String superPowerStringx = "Enemy units not on a property suffer 2 HP of damage."; //Holds the Super description
                      //"                                    " sizing markers
        String intelx = "Peter's units are exceptionally good" +
                        "against direct units, but they deal " +
                        "less damage against units on dense  " +
                        "terrain. His COP increases his day  " +
                        "to day, while hs SCOP deals 2 damage" +
                        "to all units not on properties.";//Holds CO intel on CO select menu, 6 lines max
    ...
    public int getAtk(Unit attacker, Unit defender){
        ...
        if(defender.getMinRange() == 1 && SCOP)
            return (peterstars += 120);
        if(defender.getMinRange() == 1 && COP)
            return (peterstars += 140);
        if(COP||SCOP)
            ...
            if(armies[i].getSide() != army.getSide() && armies[i].getUnits() != null){
                ...
                for(int s = 0; s < u.length; s++){
                    if(u[s].getClass() != null){
                        if(army.getBattle().getMap().find(u[s]).getTerrain().isUrban() == false && u[s].getMoveType() != u[s].MOVE_AIR)
