...
public class Alexander extends CO{
    ...
    public Alexander() {
        ...
        String skillStringx = "Alexander captures are increased by 1 ";
        ...
        String superPowerStringx = "Enemies receive no income, and half of their current funs are diverted to Alexander."; //Holds the Super description
              //"                                    " sizing markers
        String intelx = 
                "Alexander is a man who wages the    " +
                "economic war. He can capture more   " +
                "effectively, and his powers allow   " +
                "him to disrup the enemy's finances. " +
                "" +
    ...
    public void COPower(){
        ...
        for(int i = 0; i < armies.length; i++){  //Going throuhg armies
            if(armies[i].getSide() != army.getSide()) { //If the play is an opponent
                u = armies[i].getUnits();
                for(int s = 0; s < u.length; s++){
                    if(u[s].getClass() != null){
                        if(u[s].getMap().find(u[s]).getTerrain() instanceof Property){ //Find all properties that are being captured
                            ((Property) u[s].getMap().find(u[s]).getTerrain()).endCapture(); //End capture
    ...
    public void deactivateSCOP() {
        ...
        Army[] armies = army.getBattle().getArmies(); //Get all armies
        for(int i = 0; i < armies.length; i++){  //Going throuhg armies
            if(armies[i].getSide() != army.getSide()) { //If an opponent... then do the following
    ...
    public void afterAction(Unit u, int index, Unit repaired, boolean main) {
        ...
        if(main && index == 2) {
            ...
            if(P.getCapturePoints() > 1 && P.getOwner() != army) {
