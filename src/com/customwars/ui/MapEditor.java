...
import com.customwars.ui.menu.MapMenu;
import com.customwars.ui.menu.SideMenu;
import com.customwars.ui.menu.TerrainMenu;
import com.customwars.ui.menu.UnitMenu;
...
public class MapEditor extends CWScreen
{
	...
	private void drawCursorTerrain(Graphics2D g)
	{
		...
		{
		    if(((Property)ter).getOwner() == null)
		        temp = TerrainGraphics.getColoredSheet(0);
		    else
		        temp = TerrainGraphics.getColoredSheet(((Property)ter).getOwner().getColor()+1);
		} 
		else
		{
    ...
    public void saveMap(){
        ...
        try{
            ...
            for(int i = 0; i<map.getTriggers().size(); i++){
                write.writeInt(map.getTriggers().get(i).getType());
                write.writeInt(map.getTriggers().get(i).getDay());
                write.writeInt(map.getTriggers().get(i).getTurn());
                switch(map.getTriggers().get(i).getType())
                {
                    case 0:
                        //unit trigger
                    ...
                    case 1:
                        //damage trigger
                        write.writeInt(((DamageTrigger)map.getTriggers().get(i)).getX());
                        write.writeInt(((DamageTrigger)map.getTriggers().get(i)).getY());
                        write.writeInt(((DamageTrigger)map.getTriggers().get(i)).getDamage());
                        write.writeBoolean(((DamageTrigger)map.getTriggers().get(i)).isDestroy());
                }
            }
