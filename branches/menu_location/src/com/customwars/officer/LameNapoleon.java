    ...
    public void COPower() {
        ...
        for(int i = 0; i < u.length; i++) {
            if(u[i].getClass() != null) {
                if(u[i].getMoveType() == u[i].MOVE_SEA || u[i].getMoveType() == u[i].MOVE_TRANSPORT) { //if sea or transport sea unit:
                    if(!u[i].isInTransport())
                        u[i].heal(30);
                } else if(u[i].getMoveType() == u[i].MOVE_AIR) { //if air unit
                    if(!u[i].isInTransport())
                        u[i].heal(30);
...
//used to deactivate Blandie's Super CO Power the next day
    ...
    public void beforeAttack(Unit owned, Unit enemy, int damage, boolean attack) {
        if(BaseDMG.findBase(owned.getUType(), enemy.getUType(),army.getBattle().getBattleOptions().isBalance()) != -1) { //If this unit can fire with primary weapon.
            if(attack && owned.getAmmo()>0 && (owned.getMType() != owned.MOVE_AIR)) { //If this unit is not an air unit, and has primary weapon ammo left
