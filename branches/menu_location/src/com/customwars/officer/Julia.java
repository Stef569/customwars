...
public class Julia extends CO{
    ArrayList location;
	final static Logger logger = LoggerFactory.getLogger(Julia.class); 
                ...
                while(!placed) {
                    for(int s=-1*radius; s <= radius; s++){
                        for(int j=-1*offset; j <= offset; j++){
                            ...
                            if(army.getBattle().getMap().onMap(temp) && !army.getBattle().getMap().find(temp).hasUnit() && army.getBattle().getMap().find(temp).getTerrain().moveCost(((Unit)deadUnits.get(i)).getMoveType()) != -1) {
                                army.getBattle().placeUnit(army.getBattle().getMap(),army.getBattle().getMap().find(temp),((Unit)deadUnits.get(i)).getUnitType(), army);
                ...
                for(int s = 0; s < u.length; s++) { //Cycles through units
                    for(int i = 0; i < alliedUnit.length; i++) { //Cycles through allied units
                        ...
                        if(u[s].checkDisplayFireRange(alliedUnit[i].getLocation()) && !army.getBattle().getFog(u[s].getLocation().getCol(), u[s].getLocation().getRow())) {
                            u[s].setActive(true);
                            u[s].setNoUnload(true);
                            u[s].setNoJoin(true);
                            u[s].setNoLoad(true);
                            u[s].setNoExplode(true);
                            u[s].setNoWait(true);
                            break;
                        }
                    }
    ...
    public void deactivateSCOP(){
        ...
        for(int t = 0; t < armies.length; t++) { //Goes through the armies
            if(armies[t].getSide() != army.getSide() && armies[t].getUnits() != null) { //If hostile
                u = armies[t].getUnits(); //gets the units of army being targetted
                for(int s = 0; s < u.length; s++) { //Cycles through units
                    u[s].setNoUnload(false);
                    u[s].setNoJoin(false);
                    u[s].setNoLoad(false);
                    u[s].setNoExplode(false);
                    u[s].setNoWait(false);
                }//This ends the unit for loop
                //This ends the if statement checking cities
            }
