...
public class Joey extends CO {
    ...
    public int getAtk(Unit attacker, Unit defender) {
        ...
        if(SCOP) {
            firepower += attacker.getCOstore()[0]*20;
        }
        if(COP) {
            firepower += 10;
        }
        ...
        if(sustainCOP) {
            ...
            if(main) {
                for(int i = 0; i<u.length; i++) {
                    ...
                    if(u[i].getCOstore()[0] <= -1 && u[i].getUnitType() == 9) {
                        tempUnit = u[i];
                ...
                for(int i = 0; i<u.length; i++) {
                    ...
                    //Mostly everything else is left alone
                    if(u[i].getAltCOstore()[0] <= -1 && u[i].getUnitType() == 9) {
                        tempUnit = u[i];
                        destroyed = u[i].damage(150, true);
                    }
                    u[i].getCOstore()[0] = 0;
    ...
    public int getDef(Unit attacker, Unit defender) {
        ...
        if(COP && defender.getUnitType() == 9 ||  defender.getUnitType() == 21 || defender.getUnitType() ==14 || defender.getUnitType() == 27) {
            defense += 20;
        }
        if(SCOP || COP) {
            defense += 10;
    ...
    public void COPower() {
        ...
        for(int c = 0; c < army.getBattle().getMap().getMaxCol(); c++) {
            for(int r = 0; r < army.getBattle().getMap().getMaxRow(); r++) {
                ...
                if(targTile.getTerrain().getName().equals("City") && !(targTile.hasUnit()) && ((Property)targTile.getTerrain()).getOwner() == army) {
                    army.getBattle().placeUnit(army.getBattle().getMap(), targTile, 9, army);
                    targTile.getUnit().getCOstore()[0] = -1;//lole, marked for DEATH
...
//carries out Blandie's Super CO Power, called by CO.activateSCOP()
    public void superCOPower() {
        ...
        for(int i = 0; i < prop.length; i++) {
            if(prop[i].getName().equals("City") && !(prop[i].getTile().hasUnit())) {
                army.getBattle().placeUnit(army.getBattle().getMap(), prop[i].getTile(), 9, army);
                prop[i].getTile().getUnit().damage(30,false);
                ...
                prop[i].getTile().getUnit().getCOstore()[0] = -boostCounter;//lole, marked for DEATH
                boostCounter++;
        ...
        Unit[] u = army.getUnits();
        for(int i = 0 ; i<u.length; i++) {
            if(u[i].getUnitType() == 9 ||  u[i].getUnitType() == 21 || u[i].getUnitType() == 14 || u[i].getUnitType() == 27) {
                //All transports get +1 movement and that change is recorded
                u[i].setMove(u[i].getMove() + 1);
                u[i].setChanged(true);
                
                //APC and BBoat specific effects
                if(u[i].getUnitType() == 9 || u[i].getUnitType() == 21) {
                    u[i].setNoRepair(true);
                    u[i].setNoResupply(true);
                    
                    //Yar this be important, positively ID'd APCs and BBoats will
                    //not die. Of course APCs with negative IDs don't count =[
                    if(u[i].getCOstore()[0] >= 0) {
                        u[i].getCOstore()[0] = boostCounter;//lole, NOT marked for DEATH?
                        boostCounter++;
...
//used to deactivate Blandie's Super CO Power the next day
    public void deactivateSCOP() {
        ...
        for(int i = 0 ; i<u.length; i++) {
            u[i].setNoRepair(false);
            u[i].setNoResupply(false);
            ...
            //
            if(u[i].getUnitType() == 9 || u[i].getUnitType() == 21) {
                int moveVal = Math.abs(u[i].getCOstore()[0]) / 100;
                
                u[i].setMove(u[i].getMove() + moveVal);
            }
        }
    }
    public boolean canUseSpecial1(Unit owned) {
        ...
        if(tempT instanceof Property) {
            ...
            if(!owned.isNoCapture() && owned.getUnitType() == 9 && ((APC)owned).getUnitsCarried() > 0 && targProp.isCapturable()) {
    ...
    public boolean canUseSpecial2(Unit owned) {
        if(SCOP && (owned.getUnitType() == 9 ||  owned.getUnitType() == 21)) {
    ...
    public boolean canTargetSpecial2(Unit owned, Location target) {
        int boostID = (Math.abs(owned.getCOstore()[0]) % 100) - 1;
        
        Unit targUnit = army.getBattle().getMap().find(target).getUnit();
        
        if(targUnit != null && targUnit.getUnitType() != 9 && targUnit.getUnitType() != 21 && !isUnitInBoostedAtID(boostID, targUnit)) {
            if(owned.getUnitType() == 21 && owned.checkAdjacent(target) && targUnit.getArmy() == army) {
                return true;
            }
        }
        if(owned.getUnitType() == 9 && owned.getLocation().equals(target) && army.getBattle().getMap().find(target).getUnit().getArmy() == army)
            return true;
        return false;
    }
    ...
    public void useSpecial2(Unit owned, Location target) {
        ...
        if(owned.getUnitType() == 21) {
            if(army.getBattle().getMap().find(target).hasUnit() && army.getBattle().getMap().find(target).getUnit().getArmy().getSide() == army.getSide()) {
                ...
                if(u.getUnitType() != 9 && u.getUnitType() != 21) {
                    u.getCOstore()[0] += 1; //increases COstore[0] by 1
                }
        ...
        //Do NOT boost units which are already in the owned unit's boost array!
            ...
            if(north.hasUnit() && north.getUnit().getArmy() == army) {
                if(north.getUnit().getUnitType() != 9 && north.getUnit().getUnitType() != 21 && !isUnitInBoostedAtID(boostID, north.getUnit())) {
                    north.getUnit().getCOstore()[0] += 1;
                    //Adding the unit to the boosted array
            ...
            if(south.hasUnit() && south.getUnit().getArmy() == army) {
                if(south.getUnit().getUnitType() != 9 && south.getUnit().getUnitType() != 21 && !isUnitInBoostedAtID(boostID, south.getUnit())) {
                    south.getUnit().getCOstore()[0] += 1;
                    //Adding the unit to the boosted array
            ...
            if(east.hasUnit() && east.getUnit().getArmy() == army) {
                if(east.getUnit().getUnitType() != 9 && east.getUnit().getUnitType() != 21 && !isUnitInBoostedAtID(boostID, east.getUnit())) {
                    east.getUnit().getCOstore()[0] += 1;
                    //Adding the unit to the boosted array
            ...
            if(west.hasUnit() && west.getUnit().getArmy() == army) {
                if(west.getUnit().getUnitType() != 9 && west.getUnit().getUnitType() != 21 && !isUnitInBoostedAtID(boostID, west.getUnit())) {
                    west.getUnit().getCOstore()[0] += 1;
                    //Adding the unit to the boosted array
    ...
    {
        army.charge(-damage*owned.getPrice()/7000/200.0);
        enemy.getArmy().charge(damage*owned.getPrice()/7000/200.0);
    }
    }
    public void afterAction(Unit u, int index, Unit repaired, boolean main) {
        /*if(index == 5 && repaired.getUnitType() == 9 && COP) //If the unit is an APC, and a unit is loaded into one
        {
        ...
        if(SCOP && (u.getUnitType() == 9 || u.getUnitType() == 21) && index == 23) {
            ...
            if(remainingMP == -1) {
                remainingMP = u.getMove();
            }
            
            int usedMP = u.getMove() - remainingMP;
            
            if(u.getCOstore()[0] > 0) {
                u.getCOstore()[0] += (usedMP * 100);
            } else {
                u.getCOstore()[0] += (usedMP * -100);
            }
            
            u.setMove(remainingMP);
