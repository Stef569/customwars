        ...
        for(int a = 0; a < armies.length; a++) {
            if(armies[a].getSide() != army.getSide() && armies[a].getUnits() != null) {
                ...
                for(int n = 0; n < u.length; n++) {
                    if(u[n].getName().equals("MDTank") || u[n].getName().equals("Neotank")|| u[n].getName().equals("Megatank")|| u[n].getMaxRange()>1) {
                        u[n]
								.setVision(u[n].getVision() + 1);
                        u[n].getEnemyCOstore()[getStatIndex()][0] = 1;
                    }
                }
            }
        }
    ...
    public void deactivateCOP() {
        ...
        for(int a = 0; a < armies.length; a++) {
            if(armies[a].getSide() != army.getSide() && armies[a].getUnits() != null) {
                Unit[] u = armies[a].getUnits();
                for(int n = 0; n < u.length; n++) {
                    if(u[n].getEnemyCOstore()[getStatIndex()][0] == 1)
                        u[n]
								.setVision(u[n].getVision() - 1);
                }
            }
                ...
                for(int e = 0; e < enemyUnits.length; e++) {
                    ...
                    for(int m = 0; m < myUnits.length; m++) {
                        ...
                        if(rCol + rRow <= enemyUnits[e].getVision()) {
                            ...
                            logger.info("Sighting unit is = " + enemyUnits[e].getName() + " at (" + eCol + ", " + eRow + ")");
