...
public class Amy extends CO{
    ...
    public int getAtk(Unit attacker, Unit defender){
        if(attacker.getMoveType() == attacker.MOVE_HOVER) {
            if(SCOP || COP)
                return 130;
            return 120;
        }
    ...
    public int getDef(Unit attacker, Unit defender){
        if(attacker.getMoveType() != attacker.MOVE_INFANTRY && attacker.getMoveType() != attacker.MOVE_MECH && SCOP)
            return 200;
        if(COP || SCOP)
            return 110;
        return 100;
        ...
        if(main) {
            for(int i = 0; i< army.getBattle().getMap().getMaxCol(); i++) {
                for(int t = 0; t< army.getBattle().getMap().getMaxRow(); t++) {
                    if(army.getBattle().getMap().find(new Location(i,t)).getTerrain().getName().equals("Reef"))
    ...
    public void deactivateCOP(){
        ...
        if(u != null) {
            for(int i = 0; i<u.length; i++) {
                if(u[i].getUType() == 26 && u[i].isChanged()) {
                    u[i]
							.setMaxRange(u[i].getMaxRange() - 1);
                    u[i].setChanged(false);
                }
                if(u[i].getUType() == 25 && u[i].isChanged()) {
                    u[i].setMove(u[i].getMove() - 1);
                    u[i].setChanged(false);
                }
            }
    ...
    public void beforeAttack(Unit owned, Unit enemy, int damage, boolean attack) {
        ...
        if(SCOP && enemy.getMoveType() != enemy.MOVE_INFANTRY && enemy.getMoveType() != enemy.MOVE_MECH)
            counter = true;
