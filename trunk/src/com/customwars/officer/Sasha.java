    ...
    public void COPower(){
        ...
        for(int s = 0; s < armies.length; s++){
            if(armies[s].getSide() != getArmy().getSide()){
                armies[s].getCO().setStars(armies[s].getCO().getStars() - (armies[s].getCO().getStars() * i));
                if(armies[s].getCO().getStars() < 0)
                    armies[s].getCO().setStars(0);
                if(armies[s].getAltCO() != null){
                    if (army.getBattle().getBattleOptions().isBalance() == true)
                        armies[s].getAltCO().setStars(
								armies[s].getAltCO().getStars() - (armies[s].getAltCO().getStars() * i/2));
                    else
                        armies[s].getAltCO().setStars(
								armies[s].getAltCO().getStars() - (armies[s].getAltCO().getStars() * i));
                    if(armies[s].getAltCO().getStars() < 0)
                        armies[s].getAltCO().setStars(0);
                }
            }
        } 
    }
