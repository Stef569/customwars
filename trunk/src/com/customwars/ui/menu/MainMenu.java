...
*/
    ...
    for (i = 0; i < ((COList.getListing()[infono].getBio().length() / 40) + 1); i++)
  ...
  private void loadMapDisploayNames() {
    File[] mapFiles = FileSystemManager.getAllAvailableMaps().toArray(new File[0]);
    ...
    for (File mapFile : mapFiles) {
      displayNames[numMaps] = mapFile.getPath();
      ...
      if (displayNames[numMaps].substring(displayNames[numMaps].length() - 4, displayNames[numMaps].length()).equals(".map")) {
        ...
        try {
          read = new DataInputStream(new FileInputStream(displayNames[numMaps]));
