...
public class Sensei extends CO{
    ...
    public Sensei() {
        ...
        String powerStringx = "Copter firepower increases. Infantry units with 9 HP appear in all allied cities, ready to be moved."; //Holds the Power description
        String superPowerStringx = "Copter firepower increases. Mech units with 9 HP appear in all allied cities, ready to be moved."; //Holds the Super description
        //"                                    " sizing markers
    ...
    public void setChange(Unit u){
        if(u instanceof Transport)u.move++;
    }
    ...
    public void superCOPower(){
        ...
        for(int i = 0; i < prop.length; i++){
            if(prop[i].getName().equals("City") && !(prop[i].tile.hasUnit())){
                prop[i].tile.addUnit(new Infantry(prop[i].tile.getLocation().getCol(),prop[i].tile.getLocation().getRow(), army, army.getBattle().getMap()));
                prop[i].tile.getUnit().damage(10,false);
