...
public class Amy extends CO{
            ...
            if(armies[i].getSide()!=army.getSide()) {
                ...
                for(int s = 0; s<u.length; s++) {
                    if(u[s].moveType != u[s].MOVE_INFANTRY && u[s].moveType != u[s].MOVE_MECH) {
                        u[s].attackPenalty -= u[s].enemyCOstore[statIndex];
                    }
                }
            }
        }
        ...
        for(int i = 0; i<armies.length; i++) {
            if(armies[i].getSide()!=army.getSide()) {
                ...
                for(int s = 0; s<u.length; s++) {
                    if(u[s].moveType != u[s].MOVE_INFANTRY && u[s].moveType != u[s].MOVE_MECH) {
                        sustain = true;
                        u[s].attackPenalty += 20;
                        u[s].enemyCOstore[statIndex] = 20;
                    }
                }
            }
        }
        ...
        for(int i = 0; i<armies.length; i++) {
            if(armies[i].getSide()!=army.getSide()) {
                ...
                for(int s = 0; s<u.length; s++) {
                    if(u[s].moveType != u[s].MOVE_INFANTRY && u[s].moveType != u[s].MOVE_MECH) {
                        sustain = true;
                        u[s].attackPenalty += 30;
                        u[s].enemyCOstore[statIndex] = 30;
                    }
                }
            }
        }
    ...
    public void beforeAttack(Unit owned, Unit enemy, int damage, boolean attack) {
        //if Amy attacks on her SCOP, enemy counters are disabled.
        if(SCOP && attack &&enemy.moveType != enemy.MOVE_INFANTRY && enemy.moveType != enemy.MOVE_MECH)
            counter = true;
