...
public class Koshi extends CO
{
   ...
   private int negLuck;
    ...
    public Koshi()
    {
        ...
        COPName = "Quirky Berserky";
        SCOPName = "Chicken Kick";
        COPStars = 4.0;
        maxStars = 7.0;
        ...
        if(allArmies != null)
        {
           for(int i = 0; i < allArmies.length; i++)
           {
              ...
              if(testArmy.getSide() != army.getSide())
              {
                 ...
                 if(currCO != null)
                 {
                    currCO.positiveLuck += 15;
                    currCO.negativeLuck += 30;
                 }

                 if(altCO != null)
                 {
                    altCO.positiveLuck += 15;
                    altCO.negativeLuck += 30;
                 }
              }
           }
        }
              ...
              if(testArmy.getSide() != army.getSide())
              {
                 ...
                 if(allUnits != null)
                 {
                    for(int j = 0; j < allUnits.length; j++)
                    {
                       ...
                       testUnit.enemyCOstore[statIndex] = testUnit.vision - 1;
                       testUnit.vision = 1;
                       testUnit.changed = true;
                    }
                 }
              }
           }
              ...
              if(testArmy.getSide() != army.getSide())
              {
                 ...
                 if(allUnits != null)
                 {
                    for(int j = 0; j < allUnits.length; j++)
                    {
                       ...
                       if(army.getCO() == this)
                       {
                          testUnit.vision = 1 + testUnit.enemyCOstore[statIndex];
                          testUnit.enemyCOstore[statIndex] = 0;
                          testUnit.changed = false;
                       }
                       else
                       {
                          testUnit.vision = 1 + testUnit.altEnemyCOstore[statIndex];
                          testUnit.altEnemyCOstore[statIndex] = 0;
                          testUnit.changed = false;
                       }
                    }
                 }
        ...
        BaseDMG.restoreBalanceDamageTables();
          ...
          else
          {
             ...
             //Keep track of current values to restore them later
              posLuck = enemy.getArmy().getCO().positiveLuck;
              negLuck = enemy.getArmy().getCO().negativeLuck;
             enemy.getArmy().getCO().positiveLuck = 0;
             enemy.getArmy().getCO().negativeLuck = 0;
             didNegateLuck = true;
          }
    ...
    public void afterAttackAction(Unit owned, Unit enemy, boolean attack)
    {
       if(!attack && COP)
       {
          ...
          if(didNegateLuck)
          {
             enemy.getArmy().getCO().positiveLuck = posLuck;
             enemy.getArmy().getCO().negativeLuck = negLuck;
             didNegateLuck = false;
          }
       }
    }
