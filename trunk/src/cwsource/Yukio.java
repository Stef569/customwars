...
public class Yukio extends CO{
    ...
    public Yukio() {
        ...
        String skillStringx = "Attacking enemy units on properties halves the income that property returns. Funds are always hidden, while Yukio is always able to view enemy funds.";
        ...
        String superPowerStringx = "Turns terrain effects into firepower bonuses."; //Holds the Super description
                      //"                                    " sizing markers
        String intelx = "" +
                        "" +
                        "" +
        ...
        repairs = new ArrayList<Property>();
        lostIncome = new ArrayList<Integer>();
        ...
        for(int i = 0; i< armies.length; i++) {
            ...
            if(armies[i].getSide()!=army.getSide()) {
                for(int s = 0; s<u.length; s++) {
                    if(u[s].enemyCOstore[statIndex] == 1) {
                        u[s].damage(40, false);
                        u[s].paralyzed = true;
                    }
                }
...
//used to deactivate Blandie's Super CO Power the next day
    ...
    public void afterEnemyAction(Unit u, int index, Unit repaired, boolean main) {
        ...
        if(index == 15) {
            if(COP || (sustain && !main))
                u.damage(20,false);
            u.enemyCOstore[statIndex] = 1; //Used for COP calculation.
        }
        if(index == 21) {
            System.out.println(army.getBattle().getBattleOptions().getFundsLevel()/2);
            lostIncome.add(army.getBattle().getBattleOptions().getFundsLevel()/2);
            repairs.add(((Property)army.getBattle().getMap().find(u).getTerrain()));
            count.add(0);
            ((Property)army.getBattle().getMap().find(u).getTerrain()).income -= army.getBattle().getBattleOptions().getFundsLevel()/2;
    ...
    public void dayStart(boolean main) {
        ...
        if(!repairs.isEmpty())
            ...
            count.set(i, Integer.valueOf(count.get(i).intValue() + 1)); //increments this by one.
            if(count.get(i) >1) {
                repairs.get(i).income += lostIncome.get(i).intValue();
                repairs.remove(repairs.get(i));
                lostIncome.remove(lostIncome.get(i));
                count.remove(count.get(i));
            }
            }
    }
}
