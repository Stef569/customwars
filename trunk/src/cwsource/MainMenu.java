...
public class MainMenu extends JComponent{
    ...
    private boolean map;        //map select mode
    private boolean CO;         //CO select mode
    ...
    private int NUM_COS = 70;   //the number of selectable COs
    ...
    Battle tempbat = new Battle(new Map(30,20));
    private CO[] coarray = {new Andy(), new Max(),new Sami(),new Nell(),new Hachi(tempbat),new Jake(),new Rachel(),new Olaf(),new Grit(),new Colin(tempbat), new Sasha(), new Eagle(), new Drake(),new Jess(),new Javier(tempbat),new Kanbei(),new Sonja(tempbat),new Sensei(),new Grimm(),new Flak(tempbat), new Lash(tempbat),new Adder(tempbat), new Hawke(), new Sturm(tempbat), new Jugger(tempbat), new Koal(), new Kindle(), new VonBolt(), new Ember(), new Mina(), new Epoch(), new Peter(), new Sabaki(), new Edge(), new Alexis(), new Alexander(), new Graves(), new Talyx(), new Yukio(), new Ozzy(), new Edward(), new Conrad(), new Eric(), new Nana(), new Melanthe(), new Falcone(), new Julia(), new Zandra(), new Carmen(), new Carrie(), new Amy(), new Sophie(), new Mary(), new Thanatos(), new Napoleon(), new Aira(), new Koshi(), new Artemis(), new Tempest(), new Rattigan(), new Robo_Andy(), new Sanjuro(), new Xavier(), new Adam(), new Walter(), new Joey(),new Varlot(),new Eniac(),new Minamoto(), new Smitan()};
    ...
    public MainMenu(JFrame f){
        ...
        for(int i=0; i<8; i++){
            ...
            for(int j=0; j<coarray.length; j++){
                if(coarray[j].getStyle() == i){
                    armyArray[i][pos] = coarray[j];
                    pos++;
                }
            }
        }
    ...
    public void drawScreen(Graphics2D g){
        ...
        if(map)drawMapSelectScreen(g);
        
        if(CO)drawCOSelectScreen(g);
        if(info && CO)drawInfoScreen(g);
...
*/
        ...
        //Draw the main CO
        if(!altcostume)
            g.drawImage(MiscGraphics.getCOSheet(infono),175,40,175 + 225, 40 + 350, 0, 0, 225, 350, this);
        else
            g.drawImage(MiscGraphics.getCOSheet(infono),175,40,175 + 225, 40 + 350, 225, 0, 450, 350, this);
        //Draw the 'basic information'
        g.setColor(Color.black);
        /*g.setFont(new Font("Impact", Font.BOLD, 38));
        g.drawString(coarray[infono].getName(), 15,53);
        g.setColor(Color.gray);*/
        g.setFont(new Font("SansSerif", Font.PLAIN, 10));
        g.drawString("CO: " + coarray[infono].getName(), 10,20);
        g.drawString(coarray[infono].title, (coarray[infono].getName().length() + 9)*6 + 10,20);
        //CO Bio
        //This is a 'word wrap' thing, used multiple times. Listen up, I'm only going to document this once. >_>
        for(i = 0; i<((coarray[infono].getBio().length()/40)+1); i++) {//As long as i is shorter than the length, in characters, of the bio divided by 40, incremented by one.
            if(coarray[infono].getBio().length() - (i+1)*40 >= 0) { //Is there more than 40 characters left in the bio?
                ...
                g.drawString(coarray[infono].getBio().substring(i*40, (i+1)*40), 10, 40 + i*15- skip*96*scale + adjust);
            ...
            } else //If there is less than 40 characters left...
            {
                ...
                g.drawString(coarray[infono].getBio().substring(i*40), 10,40 + i*15- skip*96*scale);
            }
            if(i+1 ==((coarray[infono].getBio().length()/40)+1))
        ...
        g.drawString(coarray[infono].getHit(), 45,40 + (store-1)*15- skip*96*scale + adjust);
        store++;
        if(((store+1)*15- skip*96*scale+ adjust)<25 && ((store+1)*15- skip*96*scale+ adjust)>0)
            adjust += 25;
        g.drawImage(MiscGraphics.getMissIcon(),10 ,(store+1)*15- skip*96*scale + adjust,this);
        g.drawString(coarray[infono].getMiss(), 45,40 + (store-1)*15- skip*96*scale + adjust);
        store++;
        //This draws the power bar
        if((22 + (store)*15- skip*96*scale + adjust)<25 && (22 + (store)*15- skip*96*scale + adjust)>0)
            adjust += 25;
        //If the CO has a COP
        if(coarray[infono].getCOPStars() != -1)
            for(i = 0; i<coarray[infono].getCOPStars()+1; i++) {
            g.drawImage(MiscGraphics.getSmallStar(5),10+i*6, 22 + (store)*15- skip*96*scale + adjust, this);
            starStore = i;
            }
        //Aaand...the SCOP
        for(i = 0; i<(coarray[infono].getMaxStars()-coarray[infono].getCOPStars()); i++) {
        ...
        for(i = 0; i<((coarray[infono].getD2D().length()/40)+1); i++) //As long as i is shorter than the length, in characters, of this divided by 40 +1
        {
            if(coarray[infono].getD2D().length() - (i+1)*40 >= 0) { //Is there more than 40 characters left?
                if((40 + i*15 + store*15- skip*96*scale+ adjust)<25 && (40 + i*15 + store*15- skip*96*scale+ adjust)>0)
                    adjust += 25;
                
                g.drawString(coarray[infono].getD2D().substring(i*40, (i+1)*40), 10, 40 + i*15 + store*15- skip*96*scale + adjust);
            } else {
                if((40 + i*15 + store*15- skip*96*scale+ adjust)<25 && (40 + i*15 + store*15- skip*96*scale+ adjust)>0)
                    adjust += 25;
                g.drawString(coarray[infono].getD2D().substring(i*40), 10,40 + i*15 + store*15- skip*96*scale + adjust);
            }
            if(i+1 ==((coarray[infono].getD2D().length()/40)+1))
                store +=i;
        }
        store += 2;
        //This draws the Power
        if(coarray[infono].COPStars != -1) {
            if(((store+1) * 15- skip*96*scale+ adjust)<25 && ((store+1) * 15- skip*96*scale+ adjust)>0)
                adjust += 25;
            
            g.drawImage(MiscGraphics.getPowerIcon(), 10, (store+1) * 15- skip*96*scale + adjust, this);
            g.drawString(coarray[infono].COPName, 26, (store+2) * 15 - 2- skip*96*scale + adjust);
            
            for(i = 0; i<((coarray[infono].getCOPString().length()/40)+1); i++) //As long as i is shorter than the length, in characters, of this divided by 40 +1
            {
                if(coarray[infono].getCOPString().length() - (i+1)*40 >= 0) { //Is there more than 40 characters left?
                    if((40 + i*15 + store*15- skip*96*scale+ adjust)<25 && (40 + i*15 + store*15- skip*96*scale+ adjust)>0)
                        adjust += 25;
                    g.drawString(coarray[infono].getCOPString().substring(i*40, (i+1)*40), 10, 40 + i*15 + store*15- skip*96*scale + adjust);
                } else {
                    if((40 + i*15 + store*15- skip*96*scale+ adjust)<25 && (40 + i*15 + store*15- skip*96*scale+ adjust)>0)
                        adjust += 25;
                    
                    g.drawString(coarray[infono].getCOPString().substring(i*40), 10,40 + i*15 + store*15- skip*96*scale + adjust);
                }
                if(i+1 == ((coarray[infono].getCOPString().length()/40)+1))
                    store +=i;
            }
        ...
        g.drawString(coarray[infono].SCOPName, 26, (store+2) * 15 - 2- skip*96*scale + adjust);
        for(i = 0; i<((coarray[infono].getSCOPString().length()/40)+1); i++) //As long as i is shorter than the length, in characters, of this divided by 40 +1
        {
            if(coarray[infono].getSCOPString().length() - (i+1)*40 >= 0) { //Is there more than 40 characters left?
                if((40 + i*15 + store*15- skip*96*scale+ adjust)<25 && (40 + i*15 + store*15- skip*96*scale+ adjust)>0)
                    adjust += 25;
                g.drawString(coarray[infono].getSCOPString().substring(i*40, (i+1)*40), 10, 40 + i*15 + store*15- skip*96*scale + adjust);
            } else {
                if((40 + i*15 + store*15- skip*96*scale+ adjust)<25 && (40 + i*15 + store*15- skip*96*scale+ adjust)>0)
                    adjust += 25;
                g.drawString(coarray[infono].getSCOPString().substring(i*40), 10,40 + i*15 + store*15- skip*96*scale + adjust);
            }
            if(i+1 == ((coarray[infono].getSCOPString().length()/40)+1))
                store +=i;
        }
        //Draws tags
        store += 2;
        for(i = 0; i< coarray[infono].TagStars.length; i++) {
            if(coarray[infono].TagStars[i] > 0) {
                if(i*15 + (store+2) * 15 - 2- skip*96*scale+ adjust<25 && i*15 + (store+2) * 15 - 2- skip*96*scale+ adjust>0)
                    adjust += 25;
                g.drawString(coarray[infono].TagCOs[i], 10, i*15 + (store+2) * 15 - 2- skip*96*scale + adjust);
                for(int t =0; t<coarray[infono].TagStars[i]; t++) {
                    g.drawImage(MiscGraphics.getBigStar(5), coarray[infono].TagCOs[i].length()*5 + 15 + t*8,(store+2) * 15 - 2- skip*96*scale + adjust+ i*15 -10, this);
                }
                if(i+1 == coarray[infono].TagStars.length)
                    store +=i;
            }
        }
    ...
    public void drawCOSelectScreen(Graphics2D g){
        ...
        for(int i=1; i < 15; i++){
            ...
            if(current != null){
                ...
                g.drawImage(MiscGraphics.getCOSheet(current.getID()),2+i%3*52,61+i/3*52,2+i%3*52+48,61+i/3*52+48,0+offset,350,48+offset,398,this);
            }else{
                break;
            }
        }
        ...
        if(current != null){
            glide++;
            g.drawImage(MiscGraphics.getCOSheet(current.getID()),339+(int)(100*Math.pow(0.89,glide)),44,339+225+(int)(100*Math.pow(0.89,glide)),44+350,offset,0,offset+225,350,this);
            ...
            if(numCOs%2==1){
                g.drawImage(MiscGraphics.getCOSheet(current.getID()),166,226,166+32,226+12,144+offset,350,144+offset+32,350+12,this);
                g.drawImage(MainMenuGraphics.getCOName(),199,226,199+50,226+15,0,current.getID()*15,50,current.getID()*15+15,this);
            }else{
                g.drawImage(MiscGraphics.getCOSheet(current.getID()),166,210,166+32,210+12,144+offset,350,144+offset+32,350+12,this);
        ...
        int store = 0;
        int k;
        for(k = 0; k<((coarray[current.getID()].getIntel().length()/36)+1); k++) {//As long as k is shorter than the length, in characters, of the bio divided by 40, incremented by one.
            if(coarray[current.getID()].getIntel().length() - (k+1)*36 >= 0) { //Is there more than 40 characters left in the bio?
                //Does this intrude upon the 'sacred space' that is the "Side: Main" info?"
                //Draw the substring - 40 characters from the last area.
                g.drawString(coarray[current.getID()].getIntel().substring(k*36, (k+1)*36), 170, 98 + k*15);
                
            } else //If there is less than 40 characters left...
            {
                //Avoiding info space.
                //Drawing the rest of the substring.
                g.drawString(coarray[current.getID()].getIntel().substring(k*36), 170, 98 + k*15);
            }
        }
        
        g.setColor(Color.black);
        g.setFont(new Font("SansSerif", Font.PLAIN, 10));
        g.drawString(coarray[current.getID()].getTitle(),220,80);
    ...
    public void drawOptionsScreen(Graphics2D g){
        ...
        g.drawString("Main Screen CO: " + coarray[Options.getMainCOID()].name,10,240);
    ...
    public void pressedA(){
        if(title){
            ...
            if(cx==0 && cy==0 && numCOs % 2 == 1){
                coSelections[numCOs] = 0;
                nosecco = true;
            }else{
                if(cx==0 && cy==0)return;
                temp = armyArray[selectedArmy][cx+cy*3-1];
            }
            if(nosecco || temp != null && !(numCOs % 2 == 1 && temp.getID()==coSelections[numCOs-1]-1)){
                if(!nosecco)coSelections[numCOs] = temp.getID()+1;
            ...
            if(item == 0){
                ...
                if(Options.isAutosaveOn())Options.setAutosave(false);
                else Options.setAutosave(true);
            }else if(item==5){
            ...
            if(chooseKey){
                ...
                if(title){
                    ...
                    if(cx != 0 || cy != 0){
                        ...
                        if(temp != null)infono = temp.getID();
                    }
                    skip = 0;
                    glide = 0;
                }else if(info){
                ...
                if(CO){
                    if(info){
                        ...
                        if(cx == 0 && cy==0){
                            Random r = new Random();
                            int sel = r.nextInt(coarray.length);
                            CO sc = coarray[sel];
