...
public class Battle implements Serializable{
    ...
    public Battle(String filename, int[] coSelect, int[] sideSelect, boolean[] altSelect, BattleOptions bopt){
        ...
        for(int n = 0; n < numArmies; n++) {
            ...
            /*c = JOptionPane.showInputDialog("Army: " + n + " AI CONTROLLED? Y/N?").charAt(0);
            if(c == 'Y' || c == 'y')
    ...
    public boolean endTurn(){
                    ...
                    if(armies[i].getSide() != armies[turn].getSide()) {
                        ...
                        if(COstorecheck != null)
                            for(int t = 0; t<COstorecheck.length; t++)
                                COstorecheck[t].enemyCOstore[armies[i].getCO().statIndex] = 0;
                    }
                }
            }
        }
        ...
        if(turn >= 0 && armies[turn].getCO().cleanEnemyStoreBegin) {
            for(int i = 0; i<numArmies; i++) {
                if(armies[i].getSide() != armies[turn].getSide()) {
                    ...
                    if(COstorecheck != null)
                        for(int t = 0; t<COstorecheck.length; t++)
                            COstorecheck[t].enemyCOstore[armies[turn].getCO().statIndex] = 0;
                }
            }
        }
        ...
        if(armies[turn].isTag())
            armies[turn].unTag();
        
        //armies[turn].charge(100); //USED BY MOOGLEGUNNER TO TEST CO POWERS DO NOT REMOVE ==========================
        //lol, uh, moved here. It works better.
        if(Options.isMusicOn()){
            Music.startMusic(armies[turn].getCO().getID());
        }
    ...
    public CO getCO(int index){
        switch(index){
            ...
            case 14:
                return new Jess();
            case 15:
                return new Javier(this);
            case 16:
                return new Kanbei();
            case 17:
                return new Sonja(this);
            case 18:
                return new Sensei();
            case 19:
                return new Grimm();
            case 20:
                return new Flak(this);
            case 21:
                return new Lash(this);
            case 22:
                return new Adder(this);
            case 23:
                return new Hawke();
            case 24:
                return new Sturm(this);
            case 25:
                return new Jugger(this);
            case 26:
                return new Koal();
            case 27:
                return new Kindle();
