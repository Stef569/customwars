        ...
        for(int i = 0; i<armies.length; i++) {
            if(armies[i].getSide() != army.getSide()) {
                ...
                for(int s = 0; s<u.length; s++) {
                    if(u[s].moveType != u[s].MOVE_AIR) {
                        u[s].defensePenalty += army.getBattle().getMap().find(u[s]).getTerrain().getDef() * u[s].getDisplayHP()* 2;
                        u[s].enemyCOstore[statIndex][0] = army.getBattle().getMap().find(u[s]).getTerrain().getDef() * u[s].getDisplayHP()* 2;
                        if(army.getBattle().getMap().find(u[s]).getTerrain().isUrban() && ((Property)army.getBattle().getMap().find(u[s]).getTerrain()).cp != ((Property)army.getBattle().getMap().find(u[s]).getTerrain()).totalcp) {
                            u[s].defensePenalty += army.getBattle().getMap().find(u[s]).getTerrain().getDef() * u[s].getDisplayHP()* 1;
                            u[s].enemyCOstore[statIndex][0] += army.getBattle().getMap().find(u[s]).getTerrain().getDef() * u[s].getDisplayHP()* 1;
                        }
                    }
                }
            }
...
//used to deactivate Mary's Super CO Power the next day
    public void deactivateSCOP() {
        ...
        for(int i = 0; i<armies.length; i++) {
            if(armies[i].getSide() != army.getSide()) {
                u = armies[i].getUnits();
                for(int s = 0; s<u.length; s++) {
                    u[s].defensePenalty -= u[s].enemyCOstore[statIndex][0];
                }
            }
        }
    }
    ...
    public void dayStart(boolean main) {
        ...
        for(int u = 0; u < afflictedUnits.size(); u++) {
            ...
            afflictedUnits.get(u).noCityRepair = false;
            afflictedUnits.get(u).noRepaired = false;
        }
    ...
    public void afflictUnit(Unit enemy) {
        ...
        enemy.noCityRepair = true;
        enemy.noRepaired = true;
    }
}