...
public class BuildMenu extends ScrollMenu{
    ...
    //constructor
    public BuildMenu(boolean land, boolean sea, boolean air, boolean pipe, int funds, CO co, Battle batt, ImageObserver screen){
        ...
        if(sea){
            ...
            if(!batt.getBattleOptions().isUnitBanned(12)){
                s[i]="Sub";
                if (batt.getBattleOptions().isBalance()==true){
                    unitCost[i]=18000*co.getCostMultiplier()/100*co.getUnitCostMultiplier()[12]/100;
                } else
        ...
        if(air){
            ...
            if(!batt.getBattleOptions().isUnitBanned(23)){
                s[i]="Stealth";
                unitCost[i]=19000*co.getCostMultiplier()/100*co.getUnitCostMultiplier()[23]/100;
                if(funds >= unitCost[i]){
                    canBuy[23]=true;
                    canBuyDisplay[i]=true;
                }
    ...
    public static BuildMenu generateContext(Property p, int turn, Battle batt, ImageObserver screen){
        ...
        return new BuildMenu(land,sea,air,pipe,funds,co,batt,screen);
    }
