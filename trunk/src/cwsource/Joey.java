...
public class Joey extends CO {
    ...
    public void COPower() {
        ...
        //Not working; only drops APCs on captured properties, does
        //not include neutrals.
        /*
        Army[] armies = army.getBattle().getArmies();
         
        for(int s = 0; s<armies.length; s++)
        {
            Property[] prop = armies[s].getProperties();
            ...
            for(int i = 0; i < prop.length; i++)
            {
                if(prop[i].getName().equals("City") && !(prop[i].tile.hasUnit()))
                {
                    ...
                    prop[i].tile.getUnit().damage(20,false);
                    prop[i].tile.getUnit().setActive(true);
                    prop[i].tile.getUnit().move = 0;
                    prop[i].tile.getUnit().COstore[0] = -1;//lole, marked for DEATH
                }
            }
        }
         */
        ...
        for(int c = 0; c < army.getBattle().getMap().getMaxCol(); c++) {
            for(int r = 0; r < army.getBattle().getMap().getMaxRow(); r++) {
                ...
                if(targTile.getTerrain().getName().equals("City") && !(targTile.hasUnit())) {
                    army.getBattle().placeUnit(army.getBattle().getMap(), targTile, 9, army);
                    targTile.getUnit().damage(20, false);
    ...
    public void superCOPower() {
        ...
        for(int i = 0; i < prop.length; i++) {
            if(prop[i].getName().equals("City") && !(prop[i].tile.hasUnit())) {
                ...
                prop[i].tile.getUnit().setActive(true);
                prop[i].tile.getUnit().noLoad = true;
...
//used to deactivate Blandie's CO Power the next day
    public void deactivateCOP() {
        COP = false;
        //returns movement to spawned APCs with units in them
    }
