...
public class BattleMenu extends InGameMenu{
            ...
            default:
                if(displayItems[item].equals("End")){
                    ...
                    if(!b.getArmy(b.getTurn()).getCO().altCostume) {
                        COmovein = new Animation(b,MiscGraphics.getCOSheet(b.getArmy(b.getTurn()).getCO().getID()),2,0,0,0, 0, 225, 350,480,0,320,0,100,100,60,25,-8);//.25 second move in
                        COfadeout = new Animation(b,MiscGraphics.getCOSheet(b.getArmy(b.getTurn()).getCO().getID()),2,0,0,0, 0, 225, 350,320,0,-100,0,100,100,30,0,8);//.25 second move out
                    } else {
                        COmovein = new Animation(b,MiscGraphics.getCOSheet(b.getArmy(b.getTurn()).getCO().getID()),2,0,0,0,225, 0, 450, 350,0,320,0,100,100,60,25,-8);//.25 second move in
                        COfadeout = new Animation(b,MiscGraphics.getCOSheet(b.getArmy(b.getTurn()).getCO().getID()),2,0,0,225, 0, 450, 350,320,0,-100,0,100,100,30,0,8);//.25 second move out
                    }
                    ...
                    if(!b.getArmy(b.getTurn()).getCO().altCostume) {
                        COmovein = new Animation(b,MiscGraphics.getCOSheet(b.getArmy(b.getTurn()).getCO().getID()),1,0,0,0, 0, 225, 350,480,0,320,0,100,100,60,75,-8);//.25 second move in
                        CO = new Animation(b,MiscGraphics.getCOSheet(b.getArmy(b.getTurn()).getCO().getID()),1,0,0,0, 0, 225, 350,320,0,320,0,100,100,b.getArmy(b.getTurn()).getCO().SCOPName.length()*5,0,0);//1.5 second stay put
                        COfadeout = new Animation(b,MiscGraphics.getCOSheet(b.getArmy(b.getTurn()).getCO().getID()),1,0,0,0, 0, 225, 350,320,0,320,0,100,0,30,0,8);//.25 second move out
                    } else {
                        COmovein = new Animation(b,MiscGraphics.getCOSheet(b.getArmy(b.getTurn()).getCO().getID()),1,0,0,0,225, 0, 450, 350,0,320,0,100,100,60,75,-8);//.25 second move in
                        CO = new Animation(b,MiscGraphics.getCOSheet(b.getArmy(b.getTurn()).getCO().getID()),1,0,0,0, 225, 0, 450, 350,0,320,0,100,100,b.getArmy(b.getTurn()).getCO().SCOPName.length()*5,0,0);//1.5 second stay put
                        COfadeout = new Animation(b,MiscGraphics.getCOSheet(b.getArmy(b.getTurn()).getCO().getID()),1,0,0,225, 0, 450, 350,320,0,320,0,100,0,30,0,8);//.25 second move out
                    }
                    taunt.setup();
                    fadeinbackground.setup(true,false);
                    background.setup(true,false);
    ...
    public static BattleMenu generateContext(Battle battle, ImageObserver screen){
        ...
        if(battle.getArmy(battle.getTurn()).getCO().canCOP() && !battle.getArmy(battle.getTurn()).getCO().COP && !battle.getArmy(battle.getTurn()).getCO().SCOP && battle.getArmy(battle.getTurn()).getCO().COPStars != -1)
            COP = true;
        if(battle.getArmy(battle.getTurn()).isTag()){
        ...
        }else if(battle.getArmy(battle.getTurn()).getCO().canCOP() && !battle.getArmy(battle.getTurn()).getCO().COP){
            if(battle.getArmy(battle.getTurn()).getAltCO()!= null)swap = true;
            //COP = true;
            end = true;
        }else if(battle.getArmy(battle.getTurn()).getAltCO()!= null){
            swap = true;
            end = true;
        }else{
            end = true;
