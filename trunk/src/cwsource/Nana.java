...
//used to deactivate Blandie's Super CO Power the next day
    ...
    public void afterAttackAction(Unit owned, Unit enemy, boolean attack) {
        ...
        {
        if(attack && owned.ammo>0 && (owned.getMType() != owned.MOVE_INFANTRY || owned.getMType() != owned.MOVE_MECH))
        {
            int offset = 0;
            int radius = 0;
            ...
            if(COP && owned.ammo>0 && (owned.getMType() != owned.MOVE_INFANTRY || owned.getMType() != owned.MOVE_MECH))
            {
