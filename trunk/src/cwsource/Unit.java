...
public abstract class Unit implements Locatable, Serializable{
    ...
    //It only applies to repairs, but DOES affect the price. Price based abilities - take note! (Edge altered)
        ...
        if(t >= minRange && t <= getMaxRange()){
            if(u != null){
                if(u.getArmy().getSide() != army.getSide()){
                    ...
                    army.addFunds(army.getCO().enemySalvage * chargeDamage * u.getPrice() /10000);
                    ...
                    if(t == 1 && !destroyed && BaseDMG.find(u, this, army.getBattle().getBattleOptions().isBalance()) > -1)
                    {
                        if(t >= u.getMinRange() && t <= u.getMaxRange())
                        {
                            ...
                            //charge and salvage
                            army.charge(chargeDamage*starValue/100.0);
                            u.getArmy().charge(chargeDamage*starValue/200.0);
...
//moves the unit to the given Location, checks if the move is valid, returns ambush status
    public boolean move(Location endLoc) {
        trapper = null;
            ...
            if(!map.find(endLoc).hasUnit() || map.find(endLoc).getUnit().getArmy().getSide() != getArmy().getSide()){
                ...
                if(!unitPath.isEmpty()) {
                    ...
                    for(int s = 0; s < movement.length; s++) {
                        switch(movement[s]) {
                            case 0: //north
                                animoots[s] = new Animation(army.getBattle(),UnitGraphics.getNorthImage(this),1,0,0,0,UnitGraphics.findYPosition(this)/16*24,24,UnitGraphics.findYPosition(this)/16*24+24,curx-1,cury,curx-1,cury-16,100,100,3,0,0);
                                cury-=16;
                                break;
                            case 1: //east
                                 animoots[s] = new Animation(army.getBattle(),UnitGraphics.getEastImage(this),1,0,0,0,UnitGraphics.findYPosition(this)/16*24,24,UnitGraphics.findYPosition(this)/16*24+24,curx,cury-3,curx+16,cury-3,100,100,3,0,0);
                                curx+=16;
                                break;
                            case 2: //south
                                animoots[s] = new Animation(army.getBattle(),UnitGraphics.getSouthImage(this),1,0,0,0,UnitGraphics.findYPosition(this)/16*24,24,UnitGraphics.findYPosition(this)/16*24+24,curx,cury,curx,cury+16,100,100,3,0,0);
                                cury+=16;
                                break;
                            case 3: //west
                                animoots[s] = new Animation(army.getBattle(),UnitGraphics.getWestImage(this),1,0,0,0,UnitGraphics.findYPosition(this)/16*24,24,UnitGraphics.findYPosition(this)/16*24+24,curx,cury-3,curx-16,cury-3,100,100,3,0,0);
                                curx-=16;
                                break;
                        }
                    }
