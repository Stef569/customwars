...
//Constructor
    ...
    public int getAtk(Unit attacker, Unit defender){
        ...
        {
            if (SCOP||COP)
                return ((90/attacker.getDisplayHP())*10);
            else
                return ((80/attacker.getDisplayHP())*10);
        } else //if the unit is defending. It's a bit crude
        {
            if (SCOP||COP)
                return ((60/attacker.getDisplayHP())*10);
            else return ((50/attacker.getDisplayHP())*10);
        }
    ...
    public void dayStart(boolean main ){
        ...
        for(int s = 0; s<enemyArmy.length; s++) {
            ...
            if(u != null)
                ...
                u[t].enemyCOstore[statIndex] = i + t;
    ...
    public void afterAction(Unit u, int index, Unit repaired, boolean main) {
        if(SCOP && u.getArmy().getSide() == army.getSide()) {
            ...
            for(int i = 0; i<armies.length; i++) {
                Unit[] unit = armies[i].getUnits();
                if(armies[i].getSide() != army.getSide()) {
                    for(int s = 0; s<unit.length; s++) {
                        if(unit[s].active) {
                            unit[s].setActive(false);
                            unit[s].paralyzed = true;
                        }
                    }
                }
            }*/
    ...
    public void afterEnemyAction(Unit u, int index, Unit repaired, boolean main) 
    {
        ...
        if(turn && index != 1 && index != 20) {
            army.getBattle().getMap().move(u, (Location)(location.get(u.enemyCOstore[statIndex])));
            u.setLocation((Location)(location.get(u.enemyCOstore[statIndex])));
            u.gas = (fuel[u.enemyCOstore[statIndex]]);
            u.setActive(true);
