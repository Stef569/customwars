...
public class Carmen extends CO {
        ...
        for(int col = 0; col < army.getBattle().getMap().getMaxCol(); col++) {
            for(int row = 0; row < army.getBattle().getMap().getMaxRow(); row++) {
                if(army.getBattle().getMap().find(new Location(col,row)).getTerrain().isUrban()) {
                    army.getBattle().getMap().find(new Location(col,row)).getTerrain().newMoveSet(new double[] {0.01,0.01,0.01,0.01,0.01,-1,-1,0.01,-1,0.01});
                }
            }
        }
    }
    ...
    public void undoWarpCities() {
        for(int col = 0; col < army.getBattle().getMap().getMaxCol(); col++) {
            for(int row = 0; row < army.getBattle().getMap().getMaxRow(); row++) {
                if(army.getBattle().getMap().find(new Location(col,row)).getTerrain().isUrban()) {
                    army.getBattle().getMap().find(new Location(col,row)).getTerrain().addMoveSet(new double[] {0.99,0.99,0.99,0.99,0.99,-1,-1,0.99,-1,0.99});
                }
            }
        }
    }
