...
//Constructor
    public Jake() {
        ...
        String intelx = "Being a top tank commander, Jake's  " +
                ...
                "and movement of vehicles too.       ";//Holds CO intel on CO select menu, 6 lines max
    ...
    public int getAtk(Unit attacker, Unit defender){
        if(army.getBattle().getMap().find(attacker).getTerrain().getName().equals("Plain") && SCOP)
            return 155;
        if(COP) { //30% direct boost
                    ...
                    if(u[i].getClass() != null){
                        ...
                        if (u[i].getMinRange() > 1){
                            if(u[i].moveType == u[i].MOVE_TREAD || u[i].moveType == u[i].MOVE_TIRE){
                                ...
                                u[i].changed = true;
                                u[i].move += 2;
                            } else if(u[i].moveType != u[i].MOVE_TREAD || u[i].moveType != u[i].MOVE_TIRE){
                                u[i].maxRange++;
                                u[i].changed = true;}
                        }
                    } else
