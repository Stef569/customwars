        ...
        if(timer>=2) //Two day aspect of fuelMod
        {
            ...
            for(int i = 0; i< armies.length; i++) {
                ...
                if(armies[i].getSide() != army.getSide())
                    if(u!= null)
                        ...
                        u[t].fuelMult = 1;
                        if(u[t].enemyCOstore[statIndex]%10 != 1)
                            u[t].noRepaired = false;
                        if(u[t].enemyCOstore[statIndex]<10 )
            ...
            if(sustainSCOP) {
                ...
                for(int i = 0; i< armies.length; i++) {
                    ...
                    if(armies[i].getSide() != army.getSide())
                        for(int t = 0; t<u.length; t++) {
                        u[t].move += u[t].enemyCOstore[statIndex];
                        }
                }
            }
        }
        ...
        for(int i = 0; i< armies.length; i++) {
            ...
            if(armies[i].getSide() != army.getSide()) {
                for(int t = 0; t<u.length; t++) {
                    ...
                    if(u[t].noRepaired)
                        u[t].enemyCOstore[statIndex] = 1;
                    u[t].noRepaired = true;
                    if(u[t].noResupplied)
                        u[t].enemyCOstore[statIndex] += 10;
        ...
        for(int i = 0; i< armies.length; i++) {
            ...
            if(armies[i].getSide() != army.getSide())
                ...
                if(u[t].price>=16000 && u[t].moveType != u[t].MOVE_AIR) {
                    u[t].enemyCOstore[statIndex] = ((int)(u[t].move/2.0+.5));
