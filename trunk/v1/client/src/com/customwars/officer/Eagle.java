...
public class Eagle extends CO{
    ...
    public int getAtk(Unit attacker, Unit defender){
        ...
        if (army.getBattle().getBattleOptions().isBalance() == true){
            if(SCOP && attacker.getMoveType() == attacker.MOVE_AIR)
                return 130;
            else if(SCOP && attacker.getMoveType() == attacker.MOVE_SEA)
                return 100;
            else if(COP && attacker.getMoveType() == attacker.MOVE_AIR)
                return 65;
            else if(COP && attacker.getMoveType() == attacker.MOVE_SEA)
                return 50;
            else if(COP)
                return 55;
            else if(SCOP || (COP && attacker.getMoveType() < 3))
                return 110;
            else return 100;
        }
        //DS Mode Stats
    ...
    public void setChange(Unit u){
        if(u.getMoveType() == u.MOVE_AIR)
            u.setDailyGas(u.getDailyGas() - 2);
    }
    ...
    public void COPower(){
        ...
        for(int i = 0; i < u.length; i++)
            if(u[i].getMoveType() != u[i].MOVE_INFANTRY && u[i].getMoveType() != u[i].MOVE_MECH)
                u[i].setActive(true);
