...
public class Drake extends CO{
    ...
    public int getAtk(Unit attacker, Unit defender){
        if(attacker.moveType == attacker.MOVE_SEA){
        ...
        }else if(attacker.moveType == attacker.MOVE_AIR){
...
//used to get the defense bonus for damage calculation
    public int getDef(Unit attacker, Unit defender){
        if(army.getBattle().getBattleOptions().isBalance() && attacker.moveType == attacker.MOVE_SEA){
            if(SCOP || COP)
                return 120;
            return 110;
        }
                ...
                for(int s = 0; s < u.length; s++){
                    if(u[s].getClass() != null){
                        if(!u[s].isInTransport()){
                            ...
                            u[s].gas = u[s].getGas()/2;
                            if(!army.getBattle().getBattleOptions().isBalance())u[s].ammo = u[s].getAmmo()/2;
                        }
                    } else
                        return;
                }
