...
public class Xavier extends CO
{
   ...
   private boolean enemyAdjusted = false;
    ...
    public int getAtk(Unit attacker, Unit defender)
    {
       ...
       if(attacker.COstore[0] == -20 || attacker.altCOstore[0] == -20)
       {
          atk -= 20;
       }
    ...
    public int getDef(Unit attacker, Unit defender)
    {
       ...
       //After-image detection
       if(defender.COstore[0] == -20 || defender.altCOstore[0] == -20)
       {
          def -= 30;
       }
    ...
    public void superCOPower()
    {
        ...
        if(units != null)
        {
           for(int i = 0; i < units.length; i++)
           {
              ...
              units[i].COstore[0] = tempRow + tempCol;
           }
        }
    }   
    ...
    public void beforeAttack(Unit owned, Unit enemy, int damage, boolean attack)
    {
       ...
       if(COP && attack && owned.unitType != 0 && owned.unitType != 1)
       {
    ...
    {
       if(main && SCOP && u.COstore[0] != -20 && index != 17 && index != 15)
       {
          //  tempCol = unit.COstore[0] / modSpace
          //  tempRow = unit.COstore[0] % modSpace
          int tempCol = u.COstore[0] / modSpace;
          int tempRow = u.COstore[0] % modSpace;
          ...
          if((u.getLocation().getCol() != tempCol) || (u.getLocation().getRow() != tempRow))
          {
             ...
             targUnit.COstore[0] = -20;          //used to indicate an after-image unit
             targUnit.noJoin = true;
                        //Also disabled
                        targUnit.noLoad = true;
    ...
    public void afterAttackAction(Unit owned, Unit enemy, boolean attack)
    {
       ...
       if(enemyAdjusted)
       {
          enemy.getArmy().getCO().minNegLuck = baseMinNegLuck_enemy;
          ...
          for(int i = 0; i < units.length; i++)
          {
             ...
             if(tempUnit != null)
             {
                if(main)
                {
                   ...
                   if(tempUnit.COstore[0] == -20)
                   {
                      tempUnit.COstore[0] = 0;
                ...
                {
                   //Destroy after-images
                   if(tempUnit.altCOstore[0] == -20)
                   {
                      tempUnit.altCOstore[0] = 0;
                      lastUnitCol = tempUnit.getLocation().getCol();
                      lastUnitRow = tempUnit.getLocation().getRow();
                      destroyed = tempUnit.damage(150, true);
                   }
