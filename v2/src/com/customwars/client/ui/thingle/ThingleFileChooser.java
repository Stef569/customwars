 ...
 * @author kevin
 */
public class ThingleFileChooser implements ThingleDialog {
  /**
   * Type value indicating that the ThingleFileChooser supports an "Open" file operation.
   */
  public static final int OPEN_DIALOG = 0;

  /**
   * Type value indicating that the ThingleFileChooser supports a "Save" file operation.
   */
   ...
   */
  private ArrayList<File> pathShown = new ArrayList<File>();
  /**
  ...
  /**
   * Changes the behaviour of this dialog. There are 2 supported dialog types: Open and Save Dialog.
   */
  private int dialogType;

  public static ThingleFileChooser createOpenDialog(String title, FileFilter filter, String selectButtonText, FileChooserListener listener) {
    ThingleFileChooser chooser = new ThingleFileChooser(title, selectButtonText, listener);
    chooser.dialogType = OPEN_DIALOG;
    return chooser;
  }

  public static ThingleFileChooser createOpenDialog(String title, FileFilter filter, String selectButtonText, File initialDir, FileChooserListener listener) {
    ThingleFileChooser chooser = new ThingleFileChooser(title, selectButtonText, initialDir, listener);
    chooser.dialogType = OPEN_DIALOG;
    return chooser;
  }

  public static ThingleFileChooser createSaveDialog(String title, FileFilter filter, String selectButtonText, FileChooserListener listener) {
    ThingleFileChooser chooser = new ThingleFileChooser(title, selectButtonText, listener);
    chooser.dialogType = SAVE_DIALOG;
    return chooser;
  }

  public static ThingleFileChooser createSaveDialog(String title, FileFilter filter, String selectButtonText, File initialDir, FileChooserListener listener) {
    ThingleFileChooser chooser = new ThingleFileChooser(title, selectButtonText, initialDir, listener);
    chooser.dialogType = SAVE_DIALOG;
    return chooser;
  }
   ...
   */
  private ThingleFileChooser(String title, String select, FileChooserListener listener) {
   ...
   * @param listener   The listener to be notified of dialog events
   */
  private ThingleFileChooser(String title, String select, File currentDir, FileChooserListener listener) {
    this.title = title;
    this.select = select;
  ...
  public void selectEntry(Widget source) {
    ...
    if (dialogType == SAVE_DIALOG) {
      File file = new File(currentDir, field.getText());
      selectFile(file);
    }
  }
...
//			}
  ...
  public void changeChooser(Widget combo) {
    ...
    currentDir = pathShown.get(index);
  ...
  public void populateChooser(Widget combo) {
    ...
    for (File file : files) {
      ...
      item.setText(view.getSystemDisplayName(file));
      ...
      if (path.get(0).equals(file)) {
        displayed = true;
        String prefix = "  ";
        for (int j = 1; j < path.size(); j++) {
          item = page.createWidget("choice");
          String entry = view.getSystemDisplayName(path.get(j));
          item.setText(prefix + entry);
          combo.add(item);
          pathShown.add(path.get(j));
          prefix += "  ";
